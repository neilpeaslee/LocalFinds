defmodule LocalfindsWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    ...
    %{user: create_user!("foo@example.com", "correct horse battery"), conn: conn}
  end
  ...
  describe "log_in_user/3" do
    ...
    test "clears session when user does not match when re-authenticating", %{
      ...
      other_user = create_user!("other@example.com", "correct horse battery")
  ...
  describe "log_out_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      LocalfindsWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        LocalfindsWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
