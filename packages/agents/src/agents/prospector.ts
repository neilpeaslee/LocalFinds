...
- Exact facts go in the database via save_find: title (the business name), url (its website or OSM page), place_osm_id (the place's osm_id from list_places), score (your 0-1 ICP fit), a short summary, and a few tags.
...
5. For each business that matches the ICP:
   ...
   - Call save_find with: type "lead"; title = the business name; place_osm_id = the place's osm_id (from list_places); url = the business website (or its OSM page if it has no site — distinct from event URLs, so the url_hash won't collide with a scout find); score = your 0-1 ICP fit; a 1-2 sentence summary of why it fits; and a few lowercase tags. Quality over quantity.
